    ...
    class Auth extends CI_Controller
    {
        ...
        public function registration()
        {
            ...
            if ($this->form_validation->run() == false) {
                ...
                Daftar akun berhasil! Silahkan aktivasi email</div>');
                redirect('auth');
            }
        }
        ...
        private function _sendEmail($token, $type)
        {
            $config = [
                ...
                'smtp_user' => '12192489@bsi.ac.id',
                'smtp_pass' => '0987654321',
                'smtp_port' => 465,
                'mailtype' => 'html',
                'charset' => 'utf-8',
                'newline' => "\r\n"
            ...
            $this->email->from('12192489@bsi.ac.id', '12192489 BERNIKA IRNADIANIS IFFADA');
            $this->email->to('bernikaiffada@gmail.com');
                //$this->input->post('email'));

            if ($type == 'verify') {
                $this->email->subject('Verifikasi Akun');
                $this->email->message('Klik link berikut untuk verifikasi akun : <a href="' . base_url() . 'auth/verify?email='
                    . $this->input->post('email') . '&token=' . urlencode($token)  . '">Aktivasi disini</a>');
            } else if ($type == 'forgot') {
                $this->email->subject('Reset Password');
                $this->email->message('Klik link berikut untuk reset password : <a href="' . base_url() . 'auth/resetpassword?email='
            ...
            if ($user) {
                ...
                if ($user_token) {
                    if (time() - $user_token['date_created'] < (60 * 60 * 24)) {
                        ...
                        $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">
                ' . $email . ' berhasil diaktivasi. Login!</div>');
                        redirect('auth');
                    } else {
